...
fun main() {
    ...
    verifyAndMeasureDuration("Part One", "LBLVVTVLP") { input.parseInput().moveCrates(9000) }
    verifyAndMeasureDuration("Part One", "TPFFBDRJD") { input.parseInput().moveCrates(9001) }

}

fun String.parseInput(): Pair< List<MutableList<Char?>>,  List<List<String>>> =
...
fun Pair< List<MutableList<Char?>>,  List<List<String>>>
        .moveCrates(cratePickerModel: Int) = second.forEach { instruction ->
